...
def get_all_directory_sizes(currentDirectory: dict, directorySizeList: list):
    directorySizeList.append(get_size_of_directory(currentDirectory))
    for subdirectory in currentDirectory['subdirectories']:
        get_all_directory_sizes(subdirectory, directorySizeList)
...
if __name__ == "__main__":
    ...
    directorySizes = []
    get_all_directory_sizes(rootDir, directorySizes)
    directorySizes.sort()
    totalUsed = directorySizes[-1]
    print([size for size in directorySizes if 70000000 - totalUsed + size >= 30000000][0])
